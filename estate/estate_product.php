<?php
/**
 * Fri Dec 12 06:08:27 GMT 2008 06:08:27
 * @author  : Ping.Hu <support@infinitytesting.com.au>
 * @version : V1.0
 * ------------------------------------------------------------
 * estate control class
 * ------------------------------------------------------------
 * estate\estate_product.php
 */
...
switch($_REQUEST["cp"]){
	...
	case 'renew':
		...
		$fee = $socstoreObj->getProductFee(1, $product_feetype, $quantity, $month);        
        $arrSetting['amount'] = number_format($fee, 2, '.', '');
        $arrSetting['month'] = $product_feetype == 'year' ? 12 : ($product_feetype == 'month' ? $month : 1);
        ...
        $req['ipgForm']['request_url']	=	'/estate/?act=product&cp=_request';
        $req['history'] 				= 	"-1";
	...
	case 'product_renew':
    	...
    	$pay_type = $_REQUEST["cp"] == 'product_pay' ? 'productfee' : 'productrenew';
		$month =  1;
		$product_feetype = 'month';		
		$reviewKey = substr(md5(uniqid(mt_rand(), 1)), 0, 8);
        $arrSetting = array(
            'buyer_id' => $_SESSION['StoreID'],
            'StoreID' => $_SESSION['StoreID'],
	...
	default:
		...
		$op	=	$_REQUEST['op'];
		if (!empty($op)) {
			if ($estateObj->storeProductAddOrEditOperateOfEstate($op)) {
				if ($op == 'next') {
					$estateObj -> destroyFormInputVar();
					header('Location:/soc.php?act=signon&step=' . $stepOperate['nextStep']);
					exit;
				}elseif ($op=='del'){
					$arrTemp = $estateObj -> getFormInputVar();
					$estateObj -> destroyFormInputVar();
					header('Location:/estate/?act=product&options='.$_REQUEST['options'].'&step='.$stepOperate['step']."&msg=" . $arrTemp['msg']);
					exit;
				}elseif($op=='edit' || $op=='paynow' || $op=='renewnow' || $op=='paylater'){
					$arrTemp = $estateObj -> getFormInputVar();
					$estateObj -> destroyFormInputVar();
				...
				header('Location:/soc.php?act=signon&options='.$_REQUEST['options'].'&step='.$stepOperate['step']."&msg=".$_SESSION["pageParam"]["msg"]);
				exit;
			}
		}
		...
		$req = $estateObj -> storeProductAddOrEditOfEstate();
		...
		$objAjax	=	new ajax();
		$req['element']['jsSector']	=	$objAjax -> regFun('getSectorList',array(array('this.value',5),array('this.id',5),array(2,5),array('Select a City/ Suburb',4)));
		...
		$smarty -> assign('content', $smarty -> fetch('estate_product_save.tpl'));
		$smarty -> assign('sidebar', 0);
	...
	if($_REQUEST['multcp']){
		switch ($_REQUEST['multcp']){
			case 'delete':
				if(count($_REQUEST['ckpid'])){
					if($estateObj->mulitopeartionestate('delete',$_REQUEST['ckpid'])){
			...
			case 'publish':
				if(count($_REQUEST['ckpid'])){
					if($estateObj->mulitopeartionestate('publish',$_REQUEST['ckpid'])){
			...
			case 'unpublish':
				if(count($_REQUEST['ckpid'])){
					if($estateObj->mulitopeartionestate('unpublish',$_REQUEST['ckpid'])){
		...
		header("Location: /estate/?act=product&step=4&options=edit&msg=$multmsg");
		exit();
	}
