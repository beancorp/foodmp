				...
				<div class="form_field">
					<div style="width: 280px;" class="category_selection">
						{foreach from=$lang.seller.attribute.5.subattrib item=l key=k}
							<span><input type="radio" class="validate[required]" name="subattr5" value="{$k}" {if $req.attribute eq '5' && $req.subAttrib eq $k || $req.subattr5 eq $k }checked{/if} style="border:none" {if $isUpdate == false}onclick="selectSectionType('5', '{$k}')"{/if}/>{$l}</span>
						{/foreach}
					</div>
				</div>
			</div>
