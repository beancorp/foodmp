...
<table cellpadding="0" cellspacing="0" width="98%" id="myproducts" class="sortable">
  ...
  <thead>
    <tr>
      ...
      <th>{$lang.tt.itemname}</th>
      <th>{$lang.tt.enabled}</th>
      <th>{$lang.tt.featured}</th>
      <th>{if $req.select.sortby == 'price_asc'}<a href="/auto/?act=product&step={$req.select.step}&sortby=price_desc">{$lang.tt.price}</a><img src="/skin/red/images/arrow-down.gif" border="0" /> {elseif $req.select.sortby == 'price_desc'} <a href="/auto/?act=product&step={$req.select.step}&sortby=price_asc">{$lang.tt.price}</a><img src="/skin/red/images/arrow-up.gif" border="0" /> {else} <a href="/auto/?act=product&step={$req.select.step}&sortby=price_asc">{$lang.tt.price}</a> {/if}</th>
      
	  <th>{if $req.select.sortby == 'datec_asc'}<a href="/auto/?act=product&step={$req.select.step}&sortby=datec_desc">{$lang.tt.dateAdd}</a><img src="/skin/red/images/arrow-down.gif" border="0" />{elseif $req.select.sortby == 'datec_desc'}<a href="/auto/?act=product&step={$req.select.step}&sortby=datec_asc">{$lang.tt.dateAdd}</a><img src="/skin/red/images/arrow-up.gif" border="0" />{else}<a href="/auto/?act=product&step={$req.select.step}&sortby=datec_asc">{$lang.tt.dateAdd}</a> {/if}</th>
      <th class="unsortable">&nbsp;</th>
    </tr>
  </thead>
  <tbody>
  ...
  <tr>
    ...
    <td><ul id="icons-products">
      {if $p.status eq 0}
      <li><a href="javascript:document.location.replace('/auto/?act=product&step={$req.select.step}&sortby={$req.select.sortby}&pid={$p.pid}');void(0);" title="{$lang.but.edit}"><img src="/skin/red/images/icon-edits.gif" /></a></li>
      <li><a href="javascript:deletes('/auto/?act=product&step={$req.select.step}&op=del&sortby={$req.select.sortby}&pid={$p.pid}');void(0);" title="{$lang.but.delete}"><img src="/skin/red/images/icon-deletes.gif" /></a></li>
      ...
      {/if}
      {if $p.status eq 2}
      <li><a href="javascript:deletes('/auto/?act=product&step={$req.select.step}&op=del&sortby={$req.select.sortby}&pid={$p.pid}');void(0);" title="{$lang.but.edit}"><img src="/skin/red/images/icon-deletes.gif" /></a></li>
      <li><a href="/soc.php?cp=dispro&pre=1&StoreID={$p.StoreID}&proid={$p.pid}" target="_blank" title="{$lang.but.preview}"><img src="/skin/red/images/icon-finds.gif" /></a></li>
      {/if}
    </ul></td>
  </tr>
