...
{include_php file='include/jssppopup.php'}
  <div class="safarispace" align=center><img src="{$req.template.MainImg.name}" {$req.template.MainImg.width_attribute} {$req.template.MainImg.height_attribute} /></div>
  <h2 style="background:#{$req.template.bgcolor}">Item Detail <a href="javascript:history.go(-1)">Back</a></h2>
...
{if $l.pid} 
	...
	<div style=" float:left; width:79px;padding:5px 10px 10px 0;">
      ...
      <table cellpadding="0" cellspacing="0" style="float:right;_margin-top:0px;">
      	<tr>
        <td>
      	<div style="position:relative;width:110px;float:left">
      		<strong style="position:position:absolute;left:0;top:0;" class="{if $l.is_auction eq 'yes'}auctionit{else}buyit{/if}">
