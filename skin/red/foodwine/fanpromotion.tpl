...
{/literal}
<div style="margin:0">
<h1 class="soc-flyers">Fan Promotion</h1>
<div class="oper-emailalerts">
	<input type="button" onclick="window.open('{$smarty.const.SOC_HTTP_HOST}fanpromotion_pdf.php{if $req.info.subAttrib}?subAttrib={$req.info.subAttrib}{/if}','_blank','width=790,height=560,toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,resizable=1');void(0);" style="cursor:pointer;padding:3px;margin:3px;" value="Create PDF / Print Flyer">
</div>
<div class="clear"></div>
<div style="position:relative; width:748px; margin-bottom: 10px;">
	<img width="748" height="527" src="/flyer_images/fan_promotion/{$req.info.subAttrib}.jpg" />
	{if $req.images.mainImage.2.bname.text neq '/images/243x100.jpg' && $req.images.mainImage.2.bname.text neq '/images/242x201.jpg'}
		<div style="position:absolute; {$position_logo}">
			<img width="{if $logo_width}{$logo_width}{else}160{/if}" src="{$req.images.mainImage.2.bname.text}"/>
		</div>
	{else}
		<div style="position:absolute; {$position_logo}">
