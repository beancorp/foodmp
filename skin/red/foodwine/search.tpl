{include file="searchpanel.tpl"}
    ...
    <ul id="search-foodwine-list">
	    	...
	    	{if $product.bu_name ne ''}				
              ...
              {if $product.store_search_result_logo.text neq "/images/79x79.jpg"}
				...
				{/if}
                  <img src="{$product.store_search_result_logo.text}" alt="{$product.bu_name}" title="{$product.bu_name}" width="{$product.store_search_result_logo.width}" height="{$product.store_search_result_logo.height}" border="0" class="item" {if $product.store_search_result_logo.text neq "/images/79x79.jpg"}onmouseover="showmoreImage_fade('pid_{$product.StoreID}',true);" onmouseout="showmoreImage_fade('pid_{$product.StoreID}',false);"{/if}/>
                </a>
                <div id="pid_{$product.StoreID}" class="moreImg_css"><img src="{$product.store_search_result_logo_big.text}" style="width:{$product.store_search_result_logo_big.width}px;height:{$product.store_search_result_logo_big.height}px;"/></div><div id="pid_{$product.StoreID}_2" class="moreImg_arror"></div></div>
				...
				{if $product.store_logo.text neq "/images/79x79.jpg"}
					<img src="{$product.store_logo.text}" alt="{$product.bu_name}" title="{$product.bu_name}" width="{$product.store_logo.width}" height="{$product.store_logo.height}" border="0" class="item" onmouseover="showmoreImage_fade('pid_{$product.StoreID}',true);" onmouseout="showmoreImage_fade('pid_{$product.StoreID}',false);"/>
				{else}
					<img src="{$product.default_store_image}" alt="{$product.bu_name}" title="{$product.bu_name}" width="120" height="91" border="0"/></a>
				{/if}
                </a>
				
                <div id="pid_{$product.StoreID}" class="moreImg_css"><img src="{$product.store_logo_big.text}" style="width:{$product.store_logo_big.width}px;height:{$product.store_logo_big.height}px;"/></div><div id="pid_{$product.StoreID}_2" class="moreImg_arror"></div></div>
                <div id="context" style=" width:600px;">
				  {if $product.website_name eq '' && $product.website_url neq ''} 
					<a href="/listing.php?id={$product.StoreID}" target="_blank" class="arrowgrey">{$product.bu_name}</a>
				  {else} 
    ...
    <ul id="search-foodwine-popularize-list">
        {foreach from=$req.stores.popularize item=category key=k}
            ...
            {if $category.items and ($category.category_name ne '')}
                ...
                <li {if ($req.popularize_category_counter-1)==$k}style="border:none;"{/if}>
