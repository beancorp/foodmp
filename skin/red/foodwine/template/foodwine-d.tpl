...
<div id="seller" class="template-d">
{if $req.template.LogoDisplay eq '1'}
  ...
  {else}
	  ...
	  {else}
		<img src="{$smarty.const.SOC_HTTP_HOST}/template_images/default_logo.jpg" width="{$req.images.mainImage.2.bname.width}" />
	  {/if}
	  </div>
	  <div id="store_logo_gap" style="height:5px;"></div>
  {/if}
...
{/if}
{include file='foodwine/template/foodwine-wine-menu-list.tpl'}
</div>