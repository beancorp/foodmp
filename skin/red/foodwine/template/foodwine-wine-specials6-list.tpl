	...
	<ul style="position:relative; z-index:2;">
		{foreach from=$req.specials.items item=p key=k}
			<li class="{if $k%2 eq 0}left{else}right{/if} {if $k<2}top{else}ntop{/if}" style="z-index:{php}echo 1000-$i;$i++;{/php};">
				...
				<div class="img"><a title="{$p.item_name}" href="{$smarty.const.SOC_HTTP_HOST}{$req.info.url_bu_name}/{$p.url_item_name}"><img src="{if $p.small_image}{$p.small_image}{else}/images/80x58.jpg{/if}" width="80" height="58" alt="{$p.name}" title="{$p.name}"/></a>
				</div>
                {else}
...
<div class="clear"></div>
<div style="padding:10px 13px 0 0 ; float:right;"><strong><a href="{$smarty.const.SOC_HTTP_HOST}foodwine/index.php?act=product&cp=list&type=wine&s=1&StoreID={$req.info.StoreID}">All Specials</a>&nbsp;&nbsp;({$req.specials.total})</strong></div>
<div class="clear"></div>
