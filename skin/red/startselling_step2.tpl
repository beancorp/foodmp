{if $req.hascount eq 'hascount'}
{literal}
<!-- Google Code for Paid signup Conversion Page -->
...
var google_conversion_label = "6rSmCN70iAEQqpGy9wM"; 
//-->
</script>
<script language="JavaScript" src="http://www.googleadservices.com/pagead/conversion.js"> </script>
<noscript>
<img height="1" width="1" border="0" src="http://www.googleadservices.com/pagead/conversion/1055688874/?label=6rSmCN70iAEQqpGy9wM&amp;guid=ON&amp;script=0" />
</noscript>
{/literal}
{if $req.SV>=10}
<!--begin clixGalore code copyright 2009 --> <img src="https://www.clixGalore.com/AdvTransaction.aspx?AdID=10913&SV={$req.SV}&OID={$req.OID}" height="0" width="0" border="0"> <!--end clixGalore code -->
{/if}
{/if}
...
function changeOnlineShopping(flag)
{
	if(flag){
		...
		//$("#payment_accepted").css('display','none');
...
<h1 style="width:400px; color:#3F3480; font-size:14pt; font-weight:bold;">{if $req.attribute eq 3 && $req.subAttrib eq 1}Employer's{elseif $req.attribute eq 3 && $req.subAttrib eq 2}Job Seeker's{elseif $req.attribute eq 5}Retailer{elseif $req.subAttrib neq '2'}Seller's{elseif $req.attribute eq 1}Agent's{else}Dealer's{/if} Information</h1>
 		...
 		<div style=" clear:both; background:#ccc; height:1px; line-height:1px; margin:25px 20px 30px 0;"><img src="images/spacer.gif" alt="" /></div>


        <form name="mainForm" action="" method="POST"  onsubmit="return checkForm(this);">
	
    	<input type="hidden" name="cp" value="">
  
    	<div style="float:left; width:{if $req.attribute eq '0'}98%{else}98%{/if};">{if $req.attribute eq 5}Write some information about your business here.{else}Write some general information about yourself and your website here.{/if}<br />
    	  <br />
          <!--<span class="redButt">{$lang.EditerClew}</span><br />-->
        <textarea name="bu_desc" class="inputB" style="width:100%;height:{if $req.attribute gt 0}450px{else}350px;{/if}">{$req.bu_desc|strip_tags:false}</textarea><br />
	...
	{if ($req.attribute eq '0') or (($req.attribute eq '5') and ($req.foodwine_type eq 'food')) or $req.attribute eq '1' or $req.attribute eq '2'}
       ...
       <tr>
        ...
        <td>
                ...
                {/foreach}
                <!--
                {foreach from=$lang.Delivery item=lp key=key}
                {if $key neq 3}
                {if $key eq 4}
                <input type="checkbox" name="bu_delivery[]" class="checkbox" value="3" {if $req.bu_delivery[3]}checked{/if}/>Pickup
                {/if}
                <input type="checkbox" name="bu_delivery[]" class="checkbox" value="{$key}" {if $req.bu_delivery[$key]}checked{/if}/>
                {if $key eq 5}<strong>{/if}{$lp.text}{if $key eq 5}</strong>{/if}
                {if $key eq 5}
                    <br>
                {/if}
                {/if}
	...
	<table id="otherdetails" width="100%" cellspacing="0">
		...
		{/if}
        	...
        	<tr><td>Youtube Video<br/><a href="/soc.php?cp=youtubeinstruction" target="_blank"><img src="/skin/red/images/icon-question.gif"/></a></td>
            <td>
        ...
        {if $req.attribute eq '5'}
            ...
            <tr id="txt_PaymentMethod" style="{if $req.payments.3 && $req.sold_status}display:{else}display:none;{/if}">
                <td colspan="2" style="font-weight:bold;">Online Shopping Payment Method:</td>
            </tr>
