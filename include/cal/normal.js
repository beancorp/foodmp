...
var gsBottom="<A href='javascript:void(0)' class='BottomAnchor' onclick='if(this.blur)this.blur();if(!fSetDate(gToday[0],gToday[1],gToday[2]))alert(\"You cannot select today!\");return false;' onmouseover='return true;' >Today : "+ (giDatePos==0?(gToday[2]+" "+gMonths[gToday[1]-1]+", "+gToday[0]):(giDatePos==1?(gMonths[gToday[1]-1]+ " " +gToday[2]+", "+gToday[0]):(gToday[0]+", "+gMonths[gToday[1]-1]+" "+gToday[2])))+"</A><input type='button' value='clear' onclick='gdCtrl.value=\"\"; if(gbAutoClose){ fHideCal();}'/>";	// the content of the bottom section.
