...
class adminJokes extends common {
	var $dbcon 	= 	null;
	var $table	=	'';
	var $smarty = 	null;
	var $lang	=	null;
    ...
    */
	 ...
	 * @return array
	 */
	public function getJokesItemName(){
		$arrResult = null;

		$_query = "select id,title from ".$this->table."jokes order by title ASC";
	...
	public function getJokesItemlist($keyword=""){
		$arrResult = null;
		$tempResult = null;
		$tmpkeyword = null;
		if(trim($keyword)!=""){
		...
		$cquery = "select COUNT(*) from ".$this->table."jokes $keywhere order by title ASC";
		...
		$_query = "select id,title from ".$this->table."jokes $keywhere order by title ASC $wheresql";
		...
		$arrResult['searNum'] = $k;
		$arrResult['linkStr']  = $clsPage->get_link('soc.php?cp=jokes&jokekeywords='.urlencode($keyword), $pageSize);
		return $arrResult;
	}
	 ...
	 * @return array
	 */
	public function getJokesInfo($id=0){
		$arrResult	= null;

		$_query	=	"select * from ".$this->table."jokes where id = '$id'";
		$this -> dbcon -> execute_query($_query);
		$arrTemp = $this -> dbcon -> fetch_records(true);
		if (is_array($arrTemp)) {
			$arrResult = $arrTemp[0];
	...
	public function getJokesItemInfo($id=0,$keyword=""){
		$arrResult	= null;
		$tmpkeyword = null;
		if($keyword!=""){
			$tmpkeyword = split(' ',$keyword);
	 ...
	 * @param array $data
	 */
	public function setJokesInfo($data){
		$boolean = false;
		if (is_array($data)) {
			if(isset($data[id])&&$data[id]!=""){
				$boolean = $this -> dbcon -> update_query($this->table."jokes" , $data,"where id='$data[id]'");
			}else{
				$boolean = $this -> dbcon -> insert_query($this->table."jokes",$data);
			}
		}
		return $boolean;
	}
	
	function deleteJokesItem($id=0){
		$query = "delete from {$this->table}jokes where id={$id} limit 1 ";
		return $this -> dbcon -> execute_query($query);
	}
	
	function getlistJokes(){
		$strhtml = "<ul>";
		if(is_array($this->getJokesItemName())){
			foreach ($this -> getJokesItemName() as $pass){
				$strhtml .="<li onclick=\"xajax_displayJokesItem('$pass[id]');\" style=\"cursor:hand;\" title=\"$pass[title]\"><a href=\"#\">$pass[title]</a></li>";
			}
		}
		$strhtml .= "</ul>";
		return $strhtml;
 ...
 * @return objResponse
 */
function displayJokesItem($id){
	$lang 			= &$GLOBALS['_LANG'];
	$objAdminJokes 	= &$GLOBALS['objAdminJokes'];
	$messages		=	'';
	$objResponse = new xajaxResponse();
	$objResponse -> assign('submitButton','disabled',"disabled");

	if ($id) {
		$arrTemp = $objAdminJokes -> getJokesInfo($id);
 ...
 * @return objResponse
 */
function saveJokesItem($objForm){
	$lang 			= &$GLOBALS['_LANG'];
	$objAdminJokes 	= &$GLOBALS['objAdminJokes'];
	$messages		=	'';
	$objResponse = new xajaxResponse();
	...
	if(trim($objForm['title']) == ""){
		$objResponse ->alert("Joke title is required.");
		$objResponse -> clear('submitButton','disabled');
		return $objResponse;
	}
	if(trim($objForm['answer']) == ""){
		$objResponse ->alert("Joke content is required.");
		$objResponse -> clear('submitButton','disabled');
		return $objResponse;
	}
	...
	if($objAdminJokes -> setJokesInfo($data)){
	...
	$strhtml = $objAdminJokes->getlistJokes();
...
function deleteJokesItem($id){
	$objAdminJokes 	= &$GLOBALS['objAdminJokes'];
	$messages		=	'';
	$objResponse = new xajaxResponse();
	$arrTemp = $objAdminJokes -> getJokesInfo($id);
	if($objAdminJokes->deleteJokesItem($id)){
